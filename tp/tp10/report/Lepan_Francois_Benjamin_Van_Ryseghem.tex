...
Dans ce rapport nous allons voir comment faire pour reconnaitre des visages par le biais d'arbre de décision. 
Nous verrons comment préparer les données, la construction de l'arbre, et sur quel critère se baser pour choisir une branche de l'arbre.
 
\section{Préparation des données}

En entrée nous possédons une image faisant 660*800 pixels contenant 10 exemples de 40 visages soit 400 visages. Une première chose à faire est de créer une pile de visage afin de facilité les calcules par la suite.
...
Maintenant que nous avons cette pile d'images nous allons la séparer en deux matrices. La première contiendra les 9 premières images de chaque classes servant à la création de l'arbre et la seconde contiendra les dernières images de chaque classes pour effectuer les tests.
...
Maintenant que les données sont prêtent à l'emploi nous allons voir comment construire l'arbre de décision.

\section{Méthodologie}

Afin de construire l'arbre de décision nous allons choisir comme critère de sélection un pixel de l'image. Mais pour savoir quel pixel choisir il va falloir calculer la variation d'entropie pour chaque pixel du sous-ensemble choisi (à la première itération on prend toutes les images).

Ensuite on choisi parmi toutes les valeurs d'entropie celle qui est la plus significative (i.e la plus grande). 
...
Voici les fonctions permettant d'effectuer ces calcules:
