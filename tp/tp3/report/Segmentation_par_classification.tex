...
\end{center}
	\caption{Images texture-0 à 4 en niveau de texture puis binarisées respectivement aux seuils : 0.46, 0.285, 0.4, 0.35, 0.33}
...
Après l'exécution du code suivant on obtient la Fig.~\ref{fig:Q5-3_calcule_val_abs}
...
On constate que cette fois sur la figure~\ref{fig:Q5-3_calcule_val_abs}, en combinant les deux attributs des pixels, on réussit à obtenir dans les cinq cas une image résultante (au milieu) proche du résultat optimal (à gauche).

De ce fait, on peut en déduire que nous pouvons utiliser la combinaison des deux attributs comme attribut du pixel.

On peut aussi généraliser en disant que plus d'attributs de classification sont combinés, plus le résultat peut être fin (proche du résultat optimal).
