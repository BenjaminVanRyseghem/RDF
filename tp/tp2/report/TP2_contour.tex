...
L'exécution du code précédent fournit la Fig.~\ref{fig:1_sur_4__1_sur_8}, sur laquelle apparaissent trois contour. Il y a d'abord le contour initiale d'une forme circulaire représentée en rouge.

Il s'y trouve également le contour vert résultant du codage du contour original en ne conservant qu'un point sur quatre. 

Enfin, il y a le contour bleu représentant le codage du contour original en ne gardant cette fois qu'un point sur huit.

\begin{figure}[ht]
	\begin{center}
		\includegraphics[width=8cm]{1_sur_4__1_sur_8.png}
	\end{center}
...
Cette fonction sert à afficher la valeur d'un point de la liste des descripteurs de Fourier 
 ...
 \subsection{À quoi correspond le descripteur de Fourier $Z_0$ d'une forme décrite par son contour?}
 C'est le barycentre de la forme.
 
 On observe que si on change sa valeur on effectue une translation.
...
Le résultat de cette exécution fournit la Fig. ~\ref{fig:chgmt_z0}, on y retrouve les deux contours du cercle. Le premier en rouge est la figure initial et le deuxième contour celui du même cercle mais translater de 5.

\begin{figure}[ht]
	\begin{center}
		\includegraphics[width=8cm]{chgmt_z0.png}
	\end{center}
	\caption{rouge: contours initial \_\_ vert: 1 sur 4 \_\_ bleu: 1 sur 8}
	\label{fig:chgmt_z0}
\end{figure}
...
Lorsque nous réduisons le ratio il se trouve qu'en effet la forme se simplifie (\emph{c.f.}~Fig.~\ref{fig:reduction_fourrier})

\begin{figure}[ht]
	\begin{center}
			\includegraphics[width=8cm]{simplification_forme_desc_fourier.png}
	\end{center}
...
Et on observe bien que si on augmente \verb?dmax? on réduit bien le contour (\emph{c.f.}~Fig.~\ref{fig:reduction_chaine_cont})


\begin{figure}[ht]
	\begin{center}
     		\includegraphics[width=8cm]{reduc_chaine_cont.png}
	\end{center}
	\caption{Réduction d'une chaine de contours. rouge: contours initial \_\_ vert:  Cordes avec dmax=0.5 \_\_ bleu  Cordes avec dmax=1}
	
\end{figure}

\section{Comparaison des deux approches}

Après avoir fait quelques tests nous nous sommes rendu compte que pour des formes connexes (triangles, carres, cercles) l'algorithme des cordes est mieux car beaucoup plus rapide que celui des descripteurs de Fourier. Cependant, dans les deux cas le l'approximation est correcte (\emph{c.f.}~Fig.~\ref{fig:comp_fourier}).

\begin{figure}[ht]
	\begin{center}
		\includegraphics[width=8cm]{cmp_fourier_normale.png}
	\end{center}
	\caption{rouge: contours initial \_\_ vert: Fourier avec ratio=0.95 \_\_ bleu: Cordes avec dmax=1}
	\label{fig:comp_fourier}
\end{figure}

Mais pour ce qui est des formes non connexes (patatoïde) l'algorithme de Fourier donne un meilleur aperçu de la forme d'origine. On observe sur la figure Fig.~\ref{fig:comp_fourier} que l'algorithme des cordes fait des segment qui sont moins représentatif de la forme de base.
